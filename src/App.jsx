...
export default function App() {
  ...
  const SummaryView = () => (
    <div className="p-6 flex flex-col items-center justify-center min-h-full space-y-8 py-12">
  ...
  const ActiveWorkoutView = () => {
    // ... existing code of ActiveWorkoutView ...
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex justify-center font-sans text-neutral-200">
      <div className="w-full max-w-md bg-neutral-950 min-h-screen relative shadow-2xl overflow-hidden flex flex-col border-x border-neutral-900">
        ...
        {/* LOGO I WYBÓR PROFILU */}
        {!activeWorkout && !summaryData && (
          <header className="bg-neutral-900/80 backdrop-blur-xl p-4 border-b border-neutral-800 flex justify-between items-center sticky top-0 z-10">
        ...
        {activeWorkout && !summaryData && (
          <button 
            ...
            className="absolute bottom-6 right-6 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white px-6 py-4 rounded-full font-bold shadow-xl shadow-emerald-900/50 flex items-center gap-3 hover:scale-105 hover:-translate-y-1 transition-all z-30 border border-emerald-400/30"
          >
            <Check size={24} strokeWidth={3} /> Zakończ
          </button>
        )}
        ...
        {!activeWorkout && !summaryData && (
          <nav className="absolute bottom-0 w-full bg-neutral-900/90 backdrop-blur-xl border-t border-neutral-800 pb-safe z-20">
            <div className="flex justify-around p-2">
              <button 
                onClick={() => setActiveTab('workout')}
                className={`flex-1 flex flex-col items-center p-2 rounded-xl transition-all duration-300 ${activeTab === 'workout' ? `${theme.textMain} scale-105` : 'text-neutral-500 hover:text-neutral-300'}`}
              >
                <Play size={24} className={activeTab === 'workout' ? theme.fillMain20 : ''} />
                <span className="text-[10px] font-bold mt-1 uppercase tracking-wider">Trening</span>
              </button>
              
              <button 
                onClick={() => setActiveTab('routines')}
                className={`flex-1 flex flex-col items-center p-2 rounded-xl transition-all duration-300 ${activeTab === 'routines' ? `${theme.textMain} scale-105` : 'text-neutral-500 hover:text-neutral-300'}`}
              >
                <List size={24} />
                <span className="text-[10px] font-bold mt-1 uppercase tracking-wider">Plany</span>
              </button>

              <button 
                onClick={() => setActiveTab('history')}
                className={`flex-1 flex flex-col items-center p-2 rounded-xl transition-all duration-300 ${activeTab === 'history' ? `${theme.textMain} scale-105` : 'text-neutral-500 hover:text-neutral-300'}`}
              >
                <History size={24} className={activeTab === 'history' ? theme.fillMain20 : ''} />
                <span className="text-[10px] font-bold mt-1 uppercase tracking-wider">Historia</span>
              </button>
              
              <button 
                onClick={() => setActiveTab('library')}
                className={`flex-1 flex flex-col items-center p-2 rounded-xl transition-all duration-300 ${activeTab === 'library' ? `${theme.textMain} scale-105` : 'text-neutral-500 hover:text-neutral-300'}`}
              >
                <Dumbbell size={24} className={activeTab === 'library' ? theme.fillMain20 : ''} />
                <span className="text-[10px] font-bold mt-1 uppercase tracking-wider">Biblioteka</span>
              </button>
            </div>
          </nav>
        )}
